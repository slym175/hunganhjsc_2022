...
if ($menu) :
    ...
    <ul class="nav navbar-nav">
        <?php foreach ($menu as $menuItem) : ?>
            <?php $imgMenu = get_field('child_menu_type', $menuItem); ?>
            <li class="dropdown menu-pc <?= $imgMenu == 'list' ? 'menu-news' : '' ?>">
                        ...
                        <?php foreach ($menuItem->children as $menuItem2) : ?>
                            <li>
                                <a href="<?= $menuItem2->url ?>" title="<?= $menuItem2->title ?>">
                                    <?php $image = get_field('child_menu_image', $menuItem2) ?>
                                    <?= wp_get_attachment_image($image, 'small-thumbnail', true, array()) ?>
                                    <span class="tt"><?= $menuItem2->title ?></span>
                                    <span class="stt"><?= $menuItem2->title ?></span>
                                </a>
                            </li>
                        <?php endforeach ?>
                    </ul>
...
<ul class="nav navbar-nav navbar-right hidden-xs">
    <li class="dropdown form-search">
        <a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true"
           aria-expanded="false">
            <img class="lazyload" src="/wp-content/themes/hunganhjsc/assets/imgs/icon-search.png" alt="Tìm kiếm"
                 style="height:auto; width:24px"></a>
        <div class="dropdown-menu">
            <form action="<?= home_url() ?>" class="form-search">
                <div class="input-group d-flex align-item-center">
                    <input type="hidden" name="post_type[]" value="post">
                    <input type="hidden" name="post_type[]" value="product">
                    <input type="text" name="s" value="<?= isset($_GET['s']) && $_GET['s'] ? $_GET['s'] : '' ?>"
                           class="form-control" placeholder="Nhập tên SP cần tìm">
                    <button type="submit" class=" btn btn-link">
                        <img class="lazyload" src="/wp-content/themes/hunganhjsc/assets/imgs/icon-search.png"
